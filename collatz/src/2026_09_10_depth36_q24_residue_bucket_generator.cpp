...
static constexpr int K=36; static constexpr u64 TAG=1ULL<<63;
static u64 p2v[40],p3v[40],MOD; static int qminv[40],Q,D,P,B;
static vector<ofstream> outs; static u64 total_words,coeff_words; static vector<u64> bucket_count;
...
string file_name(string const&dir,int mask,int b){return dir+"/m46_q"+to_string(Q)+"_p"+to_string(P)+"_s"+to_string(mask)+"_b"+to_string(b)+".bin";}
Stat process_bucket(string const&dir,int b,bool do_delete){
    ...
    for(int s=0;s<shards;s++){
        ...
        if(do_delete) std::remove(fn.c_str());
    }
...
int main(int argc,char**argv){
    ...
    if(mode=="gen"){
        ...
        if(Q<23||Q>24||P<0||P>8||mask<0||mask>=(1<<P)||B<=0)return 4;
    ...
    if(mode=="process"){
        if(argc!=9)return 5;string dir;int b0,b1;init(stoi(argv[2]));P=stoi(argv[3]);B=stoi(argv[4]);b0=stoi(argv[5]);b1=stoi(argv[6]);dir=argv[7];bool del=stoi(argv[8]);
        if(Q<23||Q>24||P<0||P>8||B<=0||b0<0||b1>B||b0>=b1)return 6;
        vector<Stat> st(b1-b0);
        #pragma omp parallel for schedule(dynamic,1)
        for(int b=b0;b<b1;b++)st[b-b0]=process_bucket(dir,b,del);
