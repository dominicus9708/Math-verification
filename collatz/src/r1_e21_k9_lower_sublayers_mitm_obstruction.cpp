...
static constexpr int W=109,K=9; static u128 M=((u128)1<<W),MASK=M-1; static u128 A[8][W];
...
struct NumEnum{int z,l;u128 lo,hi;std::vector<std::vector<u128>>L;std::function<void(u128)>cb;NumEnum(int z_,u128 a,u128 b,std::function<void(u128)>c):z(z_),l(z_/2),lo(a),hi(b),L(W),cb(c){}
...
void run(){if(z==0)return;lr(0,0,0);for(auto&v:L)std::sort(v.begin(),v.end());rr(l,l,0,-1);}};
int main(){init_terms();auto F=make_filters();auto[lo,hi]=interval();uint64_t numeric=0,c15=0,c18=0,c21=0;std::vector<u128>surv21;for(int z=1;z<=7;z++){NumEnum en(z,lo,hi,[&](u128 U){numeric++;uint64_t t15=(uint64_t)((U<<K)%F.m15);if(!F.a15[t15])return;c15++;uint64_t t18=(uint64_t)((U<<K)%F.m18);if(!std::binary_search(F.T18.begin(),F.T18.end(),t18))return;c18++;uint64_t t21=(uint64_t)((U<<K)%F.m21);if(!std::binary_search(F.T21.begin(),F.T21.end(),t21))return;c21++;surv21.push_back(U);});en.run();}
if(numeric!=35245014||c15!=1585534||c18!=248162||c21!=31116)return 1;std::vector<u128>cur=std::move(surv21);const std::pair<int,uint64_t> exp[]={{24,3391},{27,326},{30,17},{33,1},{36,0}};for(auto[d,n]:exp){std::vector<u128>nx;for(u128 U:cur)if(formation(U<<K,d))nx.push_back(U);if(nx.size()!=n)return 2;cur.swap(nx);}std::cout<<"R1 E21 k9 lower sublayers: PASS\n";}
