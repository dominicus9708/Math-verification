#!/usr/bin/env python3
"""MATH-100 stage A: exact phase-danger automaton exporter for macro depth 13.

Same architecture as MATH-098/099.  Build the address-forgotten lower-envelope
backward danger corridor, restore every actual first-macro compact state that
intersects it, and emit integer-only tables for the compact carry verifier.
"""
...
TARGET=13; LAM=Fraction(19,503); EDGES=m85.EDGES; MEDGES=m86.merged_phase_edges()
...
def main():
    ...
    for fn,rows in [('t13_edges.tsv',[(i,e.H,e.Q,e.source_A,e.target_B) for i,e in enumerate(EDGES)]),('t13_roots.tsv',[(i,*r) for i,r in enumerate(roots)]),('t13_trans.tsv',trans),('t13_term.tsv',term)]:
        with open(fn,'w') as f:
            for row in rows:f.write('\t'.join(map(str,row))+'\n')
    print('PASS MATH-100 stage A')
