#!/usr/bin/env python3
# MATH-103 stage A: exact phase-danger automaton exporter for one-paid t=10.
# Same construction as MATH-102 with TARGET=10 and canonical root count 300.
...
TARGET=10;LAM=Fraction(19,503);EDGES=m85.EDGES;MEDGES=m86.merged_phase_edges()
...
@lru_cache(None)
def inv3(q,b):m=1<<b;return pow(pow(3,q,m),-1,m)
def main():
 D=merge(graph())
 def hit(d,H,p):
 ...
 for fn,rows in [('t10_edges.tsv',[(i,e.H,e.Q,e.source_A,e.target_B) for i,e in enumerate(EDGES)]),('t10_roots.tsv',[(i,*r) for i,r in enumerate(roots)]),('t10_trans.tsv',tr),('t10_term.tsv',te)]:
  with open(fn,'w') as f:
   for row in rows:f.write('\t'.join(map(str,row))+'\n')
 print('PASS MATH-103 stage A',len(roots),len(tr),len(te),len(PID))
