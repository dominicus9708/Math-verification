#include <bits/stdc++.h>
#include <omp.h>
using namespace std; using u64=uint64_t; static constexpr int K=37; static constexpr u64 TAG=1ULL<<63;
...
int main(int argc,char**argv){if(argc<3)return 2;Q=stoi(argv[1]);BT=stoi(argv[2]);int threads=argc>3?stoi(argv[3]):5;if(BT<1||BT>12||Q<=BT||Q>37)return 3;E=Q-BT;B=1;for(int i=0;i<BT;i++)B*=3;
 ...
 int b0=argc>4?stoi(argv[4]):0,b1=argc>5?stoi(argv[5]):B;if(b0<0)b0=0;if(b1>B)b1=B;if(b0>=b1)return 4;vector<int>ids;for(int b=b0;b<b1;b++)if(bn[b])ids.push_back(b);sort(ids.begin(),ids.end(),[&](int a,int b){return bn[a]>bn[b];});vector<Stat>st(B);omp_set_num_threads(threads);
 ...
 cout<<"Q "<<Q<<" BT "<<BT<<" range "<<b0<<" "<<b1<<" arbitrary_words "<<T.n<<" classes "<<T.classes<<" coefficient_words "<<T.cand<<" candidate_classes "<<T.candclasses<<" candidate_collision_excess "<<(T.cand-T.candclasses)<<" survivors "<<T.surv<<" dominated_classes "<<T.dom<<" mincredit "<<(T.minc==UINT64_MAX?0:T.minc)<<" maxcredit "<<T.maxc<<"\n";
}
