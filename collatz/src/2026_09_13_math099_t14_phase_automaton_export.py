#!/usr/bin/env python3
"""MATH-099 stage A: exact phase-danger automaton exporter for macro depth 14.

Identical architecture to MATH-098, with target depth 14.  The exporter builds
an address-forgotten lower-envelope backward corridor, restores every actual
first-macro compact state intersecting it, and emits integer-only tables for the
binary MATH-099 carry verifier.
"""
...
TARGET=14; LAM=Fraction(19,503); EDGES=m85.EDGES; MEDGES=m86.merged_phase_edges()
...
def main():
    ...
    for fn,rows in [('t14_edges.tsv',[(i,e.H,e.Q,e.source_A,e.target_B) for i,e in enumerate(EDGES)]),('t14_roots.tsv',[(i,*r) for i,r in enumerate(roots)]),('t14_trans.tsv',trans),('t14_term.tsv',term)]:
        with open(fn,'w') as f:
            for row in rows:f.write('\t'.join(map(str,row))+'\n')
    print('PASS MATH-099 stage A')
    print('roots',len(roots),'transitions',len(trans),'terminal_phase_edges',len(term),'phase_ids',len(PHASE_ID))
